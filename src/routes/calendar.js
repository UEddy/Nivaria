...
router.get('/google/callback', callbackLimiter, async (req, res) => {
  ...
  if (req.query.error) {
    ...
    return res.redirect(`/app#/profile/integrations?calendar_error=${msg}`);
  }
  ...
  if (!expected || expected.provider !== 'google' || expected.userId !== userId) {
    ...
    return res.redirect('/app#/profile/integrations?calendar_error=' + encodeURIComponent('OAuth state missing or mismatched. Start the flow again.'));
  }
  ...
  if (a.length !== b.length || !crypto.timingSafeEqual(a, b)) {
    ...
    return res.redirect('/app#/profile/integrations?calendar_error=' + encodeURIComponent('OAuth state mismatch. Possible CSRF. Start the flow again.'));
  }
  if (Date.now() - expected.issuedAt > 10 * 60 * 1000) {
    ...
    return res.redirect('/app#/profile/integrations?calendar_error=' + encodeURIComponent('OAuth flow timed out. Start the flow again.'));
  }
  if (!incomingCode) {
    oauthDebug(`[calendar:callback] REJECT reason=no_auth_code`);
    return res.redirect('/app#/profile/integrations?calendar_error=' + encodeURIComponent('No authorization code returned.'));
  }
  ...
  try {
    ...
    oauthDebug(`[calendar:callback] SUCCESS → redirecting to /app#/profile/integrations?calendar_connected=google`);
    res.redirect('/app#/profile/integrations?calendar_connected=google');
  } catch (e) {
    console.error(`[calendar:callback] EXCEPTION in token/insert path: ${e.message}\n${e.stack?.split('\n').slice(0,4).join('\n')}`);
    res.redirect('/app#/profile/integrations?calendar_error=' + encodeURIComponent(e.message.slice(0, 200)));
  }
});
