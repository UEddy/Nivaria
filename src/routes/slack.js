...
router.get('/oauth/callback', async (req, res) => {
  ...
  if (req.query.error) {
    return res.redirect('/app#/settings?slack_error=' + encodeURIComponent(String(req.query.error).slice(0, 200)));
  }
  ...
  if (!expected || expected.userId !== userId) {
    return res.redirect('/app#/settings?slack_error=' + encodeURIComponent('OAuth state missing. Start again.'));
  }
  const a = Buffer.from(expected.state, 'utf8');
  const b = Buffer.from(incomingState, 'utf8');
  if (a.length !== b.length || !crypto.timingSafeEqual(a, b)) {
    return res.redirect('/app#/settings?slack_error=' + encodeURIComponent('OAuth state mismatch. Possible CSRF.'));
  }
  if (Date.now() - expected.issuedAt > 10 * 60 * 1000) {
    return res.redirect('/app#/settings?slack_error=' + encodeURIComponent('OAuth flow timed out. Start again.'));
  }
  if (!incomingCode) {
    return res.redirect('/app#/settings?slack_error=' + encodeURIComponent('No authorization code returned.'));
  }
  ...
  try {
    ...
    res.redirect('/app#/settings?slack_connected=1');
  } catch (e) {
    console.error('[slack:callback] failed:', e.message);
    res.redirect('/app#/settings?slack_error=' + encodeURIComponent(e.message.slice(0, 200)));
  }
});
